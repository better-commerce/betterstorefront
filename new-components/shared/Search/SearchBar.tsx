...
import { buildAutocompleteQueryConfig, buildFacetConfigFromConfig, buildSearchOptionsFromConfig, getConfig, } from '@new-components/config/config-helper'
import { useUI } from '@new-components/ui'
import { SEARCH_PROVIDER } from '@new-components/utils/constants'
const ElasticSearchBar = dynamic(() => import('@new-components/shared/Search/elastic/ElasticSearchBar'))
const InstantSearchBar = dynamic(() => import('@new-components/shared/Search/algolia/InstantSearchBar'))
const SearchWrapper = dynamic(() => import('@new-components/shared/Search'))
let connector: any
...
const SearchBar: FC<React.PropsWithChildren<ISearchBarProps>> = ({ id = 'search', onClick, keywords, maxBasketItemsCount, deviceInfo, }) => {
  ...
  const defaultSearch = (
    <>
      <div className="relative items-center justify-center w-10 h-10 rounded-full sm:top-0 lg:flex sm:w-12 sm:h-12 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 focus:outline-none" onClick={onClick}>
