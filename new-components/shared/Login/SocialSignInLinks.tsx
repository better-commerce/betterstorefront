    ...
    <div className={containerCss}>
      {
        socialLogins?.includes(SocialMediaType.GOOGLE) && (
          <Link legacyBehavior href={`/my-account/login/social/${SocialMediaType.GOOGLE}`} >
            <a className="flex w-full rounded-lg bg-primary-50 dark:bg-neutral-800 px-4 py-3 transform transition-transform sm:px-6 hover:translate-y-[-2px]" >
              ...
              <h3 className="flex-grow text-sm font-medium text-center text-neutral-700 dark:text-neutral-300 sm:text-sm">
                {translate('label.login.googleLoginText')}
              </h3>
            </a>
          </Link>
        )
        ...
        socialLogins?.includes(SocialMediaType.FACEBOOK) && (
          ...
          >
            <a className={`btn w-full px-6 py-2 text-sm font-medium text-center text-white bg-sky-600 ${isLoginSidebarOpen ? `!px-0 !py-2 !pl-1 !pr-1` : ''}`}>
              ...
              {translate('label.login.facebookLoginText')}
            </a>
          </Link>
        )
      }
        ...
        socialLogins?.includes(SocialMediaType.APPLE) && (
          ...
          >
            <a className={`btn w-full px-6 py-2 text-sm font-medium text-center text-white bg-black ${isLoginSidebarOpen ? `!px-0 !py-2` : ''}`}>
              ...
              {translate('label.login.appleLoginText')}
            </a>
          </Link>
        )
      }
