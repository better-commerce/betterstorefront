...
export default function getCustomerTrades(cookies?: any, params?:any) {
  async function getCustomerTradesAsync() {
    const url = new URL(`${TRADE_IN_GET_QUOTES}?page=${params?.page || 1}&pageSize=${params?.pageSize || 10}&sortBy=${params?.sortBy || 'created_on'}&sortDescending=${params?.sortDescending || true}`, BC_API_BASE_URL)
