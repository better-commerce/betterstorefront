 ...
 */
export const ORDER_CONFIRMATION_AFTER_PROGRESS_BAR_ELEM_SELECTORS = [
  'ordconf-aftprgbar',
]

const useContentSnippet = (snippets: Array<ISnippet>, refs: any): void => {
  const injectSnippets = (snippets: Array<ISnippet>, refs: any): void => {
    let sortedSnippets = snippets?.sort((a, b) =>
      b.name < a.name ? 1 : b.name > a.name ? -1 : 0
    )
    if (sortedSnippets && sortedSnippets?.length) {
      try {
        sortedSnippets.forEach((snippet: ISnippet) => {
          if (snippet.content) {
            if (snippet.placement === SnippetPlacementType.HEAD_AFTER_OPEN) {
              // For "TopHead"

              topHead(snippet)
            } else if (
              snippet.placement === SnippetPlacementType.HEAD_BEFORE_CLOSE
            ) {
              // For "Head"

              head(snippet)
            } else if (
              snippet.placement === SnippetPlacementType.BODY_AFTER_OPEN
            ) {
              // For "BodyStartHtmlTagAfter"

              bodyStartHtmlTagAfter(snippet, refs)
            } else if (
              snippet.placement === SnippetPlacementType.BODY_BEFORE_CLOSE
            ) {
              // For "BodyEndHtmlTagBefore"

              bodyEndHtmlTagBefore(snippet, refs)
            } else if (
              snippet.placement === SnippetPlacementType.PAGE_CONTAINER_AFTER
            ) {
              // For "PageContainerAfter"

              pageContainerAfter(snippet)
            } else if (
              snippet.placement === SnippetPlacementType.HEADER_MENU_BEFORE
            ) {
              // For "HeaderMenuBefore"

              headerMenuBefore(snippet)
            } else if (
              snippet.placement === SnippetPlacementType.HEADER_MENU_AFTER
            ) {
              // For "HeaderMenuAfter"

              HeaderMenuAfter(snippet)
            } else if (
              snippet.placement === SnippetPlacementType.FOOTER_BEFORE
            ) {
              // For "FooterBefore"

              footerBefore(snippet)
            } else if (
              snippet.placement === SnippetPlacementType.FOOTER_AFTER
            ) {
              // For "FooterAfter"
              ...
              snippet.placement === SnippetPlacementType.ORDER_CONFIRMATION
            ) {
              // For "OrderConfirmationAfterProgressBar"
 ...
 * @param elements
 */
const bodyStartHtmlTagAfter = (snippet: ISnippet, refs: any) => {
  const bodyElem: any = document.querySelector('.promotion-banner')
 ...
 * @param elements
 */
const bodyEndHtmlTagBefore = (snippet: ISnippet, refs: any) => {
...
const productAndBrandDescription = (snippet: ISnippet) => {
  const parentElem: any = document.querySelector(
    `.${ELEM_ATTR}${PDP_ELEM_SELECTORS[0]}`
  )
...
const orderConfirmationAfterProgressBar = (snippet: ISnippet) => {
  const parentElem: any = document.querySelector(
    `.${ELEM_ATTR}${ORDER_CONFIRMATION_AFTER_PROGRESS_BAR_ELEM_SELECTORS[0]}`
  )
  const attrs = buildAttrs([
    ORDER_CONFIRMATION_AFTER_PROGRESS_BAR_ELEM_SELECTORS[0],
  ])
  insertAdjacentHTML(
    snippet.content,
    snippet.type,
