...
export default function getCategoryProductsOperation() {
   ...
   */
  async function getCategoryProducts(categoryId: string, cookies?: any) {
    try {
      const response: any = await fetcher({ url: SEARCH_ADVANCED_ENDPOINT, method: 'post', data: { categoryId }, cookies, })
