...
export default function useClickCollectStore() {
    return async function handler({ basketId, deliveryMethodId, store, cookies, }: Props) {
        const data = { slots: null, id: store?.Id, name: store?.Name, externalRefId: store?.StoreId, code: null, stockCode: null, company: null, address1: store?.Address1, address2: store?.Address2, city: store?.City, county: null, postCode: store?.PostCode, country: null, countryCode: null, distance: null, availableStock: 0, leadTime: 0, leadTimeUom: 0, latitude: null, longitude: null, phoneNo: null, mobileNo: null, leadTimeMin: store?.LeadTime, leadTimeMax: store?.LeadTimeUom, distanceFromPostCode: (store?.Distance || store?.DistanceInMetres), leadTimeUnit: "Days", deliveryOption: "orderIntoStore", shippingPlanId: deliveryMethodId, openingHours: null, timeslotActual: null, selectedSlot: null, image: null, distanceUnit: null, hdnCode: null, YourId: store?.StoreId }
        try {
