...
export default function Carousel({ images }: any) {
  ...
  return (
    <div className="relative w-full px-4 mx-auto">
      ...
      <p className="mt-2 text-sm font-normal text-center text-gray-600"> {images[currentIndex]?.description} </p>

      <div className="relative flex items-center justify-center overflow-hidden bg-transparent border-b border-[#2d4d9c] rounded-full mx-auto w-[372px] h-[372px]" onTouchStart={handleTouchStart} onTouchMove={handleTouchMove} onMouseDown={handleMouseDown} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} >
      ...
      <div className="flex justify-center mt-6 space-x-2">
        {images?.map((_: any, index: number) => (
          <button key={index} onClick={() => goToSlide(index)} className={`w-3 h-3 rounded-full transition-colors ${index === currentIndex ? "bg-[#1a3067]" : "bg-gray-400 hover:bg-red-600"}`} />
        ))}
      </div>
    </div>
  );
