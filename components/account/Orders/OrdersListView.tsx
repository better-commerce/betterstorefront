  ...
  return (
    isShowDetailedOrder ? (
      <div id="OrderDetail" className="w-full">
        <OrderDetail show={isShowDetailedOrder} showDetailedOrder={setIsShowDetailedOrder} details={orderDetails} deviceInfo={deviceInfo} />
      </div>
    ) : (
      <div className="bg-white dark:bg-transparent">
        <div className="max-w-4xl">
          {!ordersList ? (<Spinner />) : (ordersList?.length > 0 ? (
            <section aria-labelledby="recent-heading" className="pt-5 mt-2" >
              ...
              <div className="w-full mx-auto overflow-hidden sm:px-4 lg:px-0 paged-orders">
                <InfiniteScroll
                  ...
                  total={ordersList?.length ? ordersList[0]?.totalRecord : 0}
                ...
                />
                {ordersList[0].totalRecord > 10 && ordersList[0].totalRecord > ordersList?.length && (
                  <div className="flex justify-center flex-1 mx-auto">
                    <button className="px-6 py-2 font-semibold text-center text-gray-700 bg-gray-100 border border-gray-200 text-14 hover:bg-gray-800 hover:text-white" onClick={handleInfiniteScroll} > {translate('common.label.loadMoreBtnText')} </button>
                  </div>
                )}
              </div>
            </section>
            ...
            <div className="flex flex-col w-full px-4 py-12 max-acc-container sm:px-0">
              <h1 className="my-2 text-2xl font-semibold text-black dark:text-white">{translate('label.orderDetails.noOrderFoundHeadingText')}</h1>
              <div className="flex mt-5 w-60 sm:flex-col">
                <Link legacyBehavior passHref href="/">
                  <a className="w-full flex items-center justify-center px-4 py-3 -mr-0.5 rounded-sm sm:px-6 link-button btn-primary">{translate('label.orderDetails.startShoppingBtnText')} </a>
                </Link>
              </div>
            </div>
          ))}
