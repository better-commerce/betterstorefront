      ...
      <>
         <div className="flex flex-col p-4 border-b sm:flex-row sm:justify-between border-slate-200 rounded-t-2xl sm:items-center sm:p-4 bg-slate-50 dark:bg-slate-500/5">
            ...
            <div className="mt-3 sm:mt-0">
               <button type="button" className="nc-Button relative h-auto inline-flex items-center justify-center rounded-full transition-colors text-sm font-medium py-2.5 px-4 sm:px-6  ttnc-ButtonSecondary bg-white text-slate-700 dark:bg-white dark:text-slate-700 hover:bg-gray-100 dark:hover:bg-slate-100  border border-slate-300 dark:border-slate-700  focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-6000 dark:focus:ring-offset-0" > {translate('label.order.viewOrderBtnText')}</button>
            </div>
         </div>
         {/* top info end */}
         ...
         <div className='w-full p-4 order-info-bottom'>
            ...
            {order.allowedToTrack &&
               <div className='w-full mt-2 acc-btn-sec'>
                  <a onClick={() => trackPackage(order)} target="_blank" rel="noopener noreferrer" href={`https://track.damensch.com/?waybill=${order.trackingNo}`} className='inline-block px-8 py-1 text-sm font-semibold text-center rounded-2xl btn btn-primary'>Track Package {idx + 1}</a>
               </div>
            }
         </div>
      </>
