...
const OrderItems = ({ items, details, ifCancelled, openHelpModal, setReview }: any) => {
   return (
      <>
            ...
            {items?.map((productItem: any, odx: number) => {
               ...
               return (
                  productItem?.price?.raw?.withTax == 0 ? (
                     ...
                     <>
                        <li className="inline-block w-full px-4 pb-0 my-4 sm:px-16" key={odx}>
                           <div className="flex gap-3 py-6 sm:gap-6">
                              ...
                              <div className="flex flex-col flex-1 flex-width-nob">
                                 ...
                                 <div className="flex mt-3">
                                    <div className="w-24">
                                       <label className="font-medium capitalize text-sm text-primary dark:text-black">Size: <span className="uppercase font-medium">{productItem?.size}</span></label>
                                    </div>
                                    <div className="w-full">
                                       <label className="font-medium text-sm text-primary dark:text-black">Qty: {productItem?.qty}</label>
                                    </div>
                                 </div>
                              </div>
                           </div>
                           {matchStrings(details?.order.orderStatus, OrderStatus.DELIVERED, true) &&
                              <div className='flex w-full'>
                                 <p className='w-16 mt-2 text-xs text-black opacity-60' onClick={() => setReview(productItem)}>Rate Item: </p>
         ...
         {details?.order.allowedToTrack ? <TrackingDetail deliveryPlan={details?.order?.deliveryPlans} /> : null}
      </>
   )
}
