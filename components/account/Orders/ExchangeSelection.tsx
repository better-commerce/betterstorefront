...
import { /*CancelOrderPageAction,*/ NEXT_GET_PRODUCT, PRODUCTS_SLUG_PREFIX } from '@components/utils/constants';
// import Spinner from '@components/ui/Spinner';
// import SubmitButton from '@components/common/SubmitButton';

const ExchangeSelection = ({ btnTitle, item, reason, selectedImages, onGetProduct, onItemExchange, submitState, deviceInfo }: any) => {
    ...
    const attributeChanged = (fieldCode: string, value: string, colorItem: any,) => {
        ...
        if (matchStrings(fieldCode, SIZE_ATTRIBUTE, true)) {
            ...
            if (colorItem?.slug) {
                ...
                const handleAsync = async (slug: string) => {
                    const { data: difColoredProd }: any = await axios.post(NEXT_GET_PRODUCT, { slug: slug });
                    //console.log("difColoredProd?.product?.variantProducts", difColoredProd?.product?.variantProducts);

                    let selectedSize = product?.customAttributes?.filter((attr: any) => attr.key === SIZE_ATTRIBUTE)[0]?.value;
                    if (selectedValue && selectedValue[SIZE_ATTRIBUTE]) {
                        selectedSize = selectedValue[SIZE_ATTRIBUTE];
                    }

                    //console.log("selectedSize", selectedSize);
                    const findProductMatch = difColoredProd?.product?.variantProducts?.find((x: any) => matchStrings(x?.attributes?.find((attr: any) => matchStrings(attr?.fieldCode, SIZE_ATTRIBUTE, true))?.fieldValue, selectedSize, true));
                ...
                <div className='mx-auto cancel-continer'>

                    {
                        !product ? (
                            ...
                            <>
                                <h4 className='mb-4 text-xl font-bold text-black'>Choose Size & Color</h4>
                                <p className='text-base text-primary'>Select color and size of the item you want in exchange</p>
                                ...
                                <div className='w-full mt-4'>
                                    ...
                                    <div className='w-full'>
                                        <div className='py-4'>
                                            <button
                                                type='button'
                                                className={`block w-full px-8 text-center text-white bg-black border btn-basic-property hover:bg-gray-800 ${!product?.productId ? "opacity-50 cursor-not-allowed" : ""}`}
                                                onClick={openModal}
                                    ...
                                    <div className="relative z-50 flex flex-col h-full">
                                        <div className="w-full sm:z-10 sm:left-1 sm:top-1">
                                            <div className='flex justify-between px-4 py-3 border-b'>
                                                <div>
                                                    <h3 className="text-base font-bold text-black dark:text-black">Exchange item </h3>
                                                </div>
                                                <button
                                                    type="button"
                                            ...
                                            <div className="w-full p-6 overflow-y-auto">
                                                <div className='w-full'>
                                                    <p className='text-sm text-black'>Are you certain you want to exchange this item? {/*You'll miss out on savings of ₹648!*/}</p>
                                                </div>
                                                <div className='w-full'>
                                                </div>
