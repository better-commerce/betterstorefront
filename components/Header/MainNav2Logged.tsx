...
import { ChevronDownIcon, HeartIcon, StarIcon } from "@heroicons/react/24/outline";
                ...
                <div className="flex items-center justify-end gap-4">
                  ...
                  <span className="relative flex items-center gap-1 pl-4 text-xs font-light text-white cursor-pointer pc-text-header-clr hover:underline" onClick={() => { handleWishlist(); }}>Wishlist <HeartIcon className="w-3 h-3" aria-hidden="true" aria-label="Wishlist" />
                    {wishListItems?.length > 0 && delayEffect && (
                      <span className="w-3.5 h-3.5 flex items-center justify-center bg-red-500 absolute top-0 -right-2.5 rounded-full text-[10px] leading-none text-white font-medium">{wishListItems?.length}</span>
                    )}
                  </span>
            ...
            {!isMobile &&
              <div className="w-full mt-2 bg-header-nav-clr">
                <div className="container flex-[2] justify-center lg:flex custom-padding-nav">
                  {featureToggle?.features?.enableForPCSite ? (<NavigationRows subMenuPosition={classTop} navItems={config} featureToggle={featureToggle} />
                  ) : (
