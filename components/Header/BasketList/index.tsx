// Base Imports
import React from 'react'

// Package Imports
import Link from "next/link";

// Other Imports
import { useUI } from '@components/ui';
import { Guid } from '@commerce/types';
import { TrashIcon } from '@heroicons/react/24/outline';

const BasketList = ({ baskets, openMiniBasket }: any) => {
    const { setBasketId } = useUI()

    if (!baskets || (baskets && !baskets?.length)) {
        return null
    }

    return (
        (baskets?.length > 0) && (
            baskets?.map((basket: any) => (
                <Link key={basket?.id} title={!basket?.name ? basket?.id.substring(0, 8) : basket?.name} passHref href="#" className="flex items-center p-2 -m-3 transition duration-150 ease-in-out rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-700 focus:outline-none focus-visible:ring focus-visible:ring-orange-500 focus-visible:ring-opacity-50" onClick={(ev: any) => {
                    ev.preventDefault()
                    ev.stopPropagation()
                    if (basket?.id && basket?.id !== Guid.empty) {
                        setBasketId(basket?.id)
                        openMiniBasket(basket)
                    }
                }}>
                    <div className={`flex items-center justify-center flex-shrink-0 capitalize text-neutral-500 dark:text-neutral-300`}>
                        {basket?.lineItems?.length > 0 && (
                            <div className="w-6 h-6 flex items-center justify-center bg-primary-500 top-1.5 right-1.5 rounded-full text-[14px] leading-none text-white font-medium">
                                {basket?.lineItems?.length}
                            </div>
                        )}
                    </div>

                    <div className='ml-4'>
                        <p className="text-sm font-medium capitalize w-20">{!basket?.name ? basket?.id.substring(0, 8) : basket?.name}</p>
                    </div>

                    <div className='flex items-center justify-center flex-shrink-0 capitalize text-neutral-500 dark:text-neutral-300 px-6'>
                        <TrashIcon className="w-5 h-5 text-black stroke-2" />
                    </div>
                </Link>
            ))
        )
    )
}

export default BasketList