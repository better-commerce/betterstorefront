  ...
  return (
    <div key={product?.id} className="p-4 border border-gray-200 rounded-md sm:pr-2" >
      <div className="grid items-start grid-cols-12 gap-4">
        <div className="col-span-3">
          <Link href={`/${product?.slug}`}>
            <img width={100} height={100} style={css} src={generateUri(product?.image, 'h=120&fm=webp') || IMG_PLACEHOLDER} alt={product?.name} className="object-cover object-center w-full h-full" onClick={handleRedirectToPDP} />
          </Link>
        </div>
        <div className="col-span-6">
          <h5 onClick={handleClose} className="font-normal font-14 dark:text-black">
        ...
        <div className="col-span-12 mt-2">
          <div className="grid items-center grid-cols-6 col-span-12 gap-4 mt-2">
            <div className="flex items-center justify-between col-span-2 px-2 border border-gray-300 rounded-md hover:border-gray-600 group hover:text-blue dark:text-black">
              {basketItem?.qty <= 1 ? (
                <button onClick={() => onUpdateKitBasket(product, CART_ACTION_TYPES.DELETE)}>
                  <TrashIcon className="w-4 text-gray-500 stroke-2 group-hover:text-red-500" title="Remove from Cart" />
                </button>
              ) : (
                <MinusIcon onClick={() => onUpdateKitBasket(product, CART_ACTION_TYPES.DECREASE)} className="w-4 text-gray-500 cursor-pointer group-hover:text-gray-900" />
              )}
              <span className="px-2 py-2 text-md">{product?.qty}</span>
              <PlusIcon className="w-5 text-gray-500 cursor-pointer group-hover:text-gray-900" onClick={() => onUpdateKitBasket(product)} />
            </div>
            <div className="flex justify-end col-span-4 font-size-sm">
              <button type="button" className="text-gray-400 hover:text-red-500" onClick={() => onUpdateKitBasket(product, CART_ACTION_TYPES.DELETE)}><TrashIcon className='w-5 h-5' /></button>
            </div>
          </div>
        </div>
      </div>
