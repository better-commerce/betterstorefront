...
import { getCurrentPage, vatIncluded, getCartValidateMessages, isB2BUser, } from '@framework/utils/app-util'
...
const CartSidebarView: FC<React.PropsWithChildren<IExtraProps>> = ({ deviceInfo, maxBasketItemsCount, config, }: any) => {
  ...
  const { addToWishlist, openWishlist, setAlert, setSidebarView, closeSidebar, setCartItems, cartItems, basketId, openLoginSideBar, user, isGuestUser = true, displaySidebar, } = useUI()
              ...
              <Transition.Child as={Fragment} enter="transform transition ease-in-out duration-500 sm:duration-700" enterFrom="translate-x-full" enterTo="translate-x-0" leave="transform transition ease-in-out duration-500 sm:duration-700" leaveFrom="translate-x-0" leaveTo="translate-x-full" >
                <div className="w-screen max-w-md mob-width-full mobile-cart-width">
                  <div className="flex flex-col h-full overflow-y-scroll bg-white shadow-xl">
                    ...
                    {!isEmpty && (
                      <div className="px-5 text-sm divide-y mt-7 text-slate-500 dark:text-slate-400 divide-slate-200/70 dark:divide-slate-700/80">
                        <div className="flex justify-between py-2 text-sm text-gray-900">
                          <p className='text-sm'> {' '} {isIncludeVAT ? translate('label.orderSummary.subTotalTaxIncsubTotalVATIncTextText') : translate('label.orderSummary.subTotalVATExText')}{' '} </p>
                    ...
                    {(!user?.userId || (user?.userId && (!isB2BUser(user) || (isB2BUser(user) && user?.canPlaceOrder))) || isGuestUser) && <div className="sticky bottom-0 z-10 w-full p-4 bg-white border-t shadow">
