...
const CustomCacheBuster = ({ buildVersion }: IProps) => {
    ...
    const getBuildNumber = (version: string) => {
        ...
        if (arrNo?.length) {
            return arrNo?.map((x: string) => parseInt(x))?.reduce((partialSum, a) => partialSum + a, 0);
        }
        return 0;
    };
    ...
    useEffect(() => {
        ...
        if (cachedVersion) {
            ...
            if (getBuildNumber(cachedVersion) < getBuildNumber(buildVersion)) {
                ...
                cache.put(CURRENT_VER_KEY, buildVersion, getMinutesInDays(100 * 365) * 60 * 1000);
                emptyCache();
                setTimeout(() => {
    ...
    return (
        ...
        <>
            ...
            <span className="data-ver" style={{ position: "relative", display: "none" }}>{buildVersion}</span>
        </>
