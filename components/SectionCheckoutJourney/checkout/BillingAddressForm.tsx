  ...
  const handleSelectAddress = async (address: any) => {
    ...
    if (foundAddress) {
      ...
      if (foundAddress?.country) {
        if (!billingCountries?.find((item: any) => item?.name === foundAddress?.country)) {
          newFoundAddress.country = billingCountries?.find((item: any) => item?.twoLetterIsoCode === foundAddress?.countryCode)?.name
        }
      }
