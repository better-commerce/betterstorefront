        ...
        {({ open }) => (
          <>
            <Disclosure.Button className="flex items-center justify-between w-full gap-2 p-3 text-sm font-light text-left text-black normal-case border-b border-gray-300 bg-gray-50">
              ...
              <span className="font-semibold text-black">{data?.grandTotal?.formatted?.withTax}</span>
            </Disclosure.Button>
            <Disclosure.Panel className="px-0 pt-3 pb-0 bg-gray-100">
              <div className="h-auto border-b border-gray-300 card-summary">
                        ...
                        if (product?.length) {
                          return (
                            <div className="checkout-summary" key={`summary-${product?.productId}`} >
                              <BasketGroupProduct key={product?.productId} products={product} />
                            </div>
                          )
                        }
                        return (
                              ...
                              <div className="col-span-9">
                                ...
                                <div className="flex items-center justify-between w-full my-2 gap-y-3">
                                  <div className="justify-start text-left">
                                    {product?.price?.raw?.withTax > 0 ? (
                                      <span className="block font-semibold text-black font-14 dark:text-black">
                                        {isIncludeVAT ? product?.price?.formatted?.withTax : product?.price?.formatted?.withoutTax} {product?.listPrice?.raw.withTax > 0 && product?.listPrice?.raw.withTax != product?.price?.raw?.withTax ? (
                                          <span className="pl-2 font-normal text-gray-400 line-through font-14"> {' '} {isIncludeVAT ? product?.listPrice.formatted?.withTax : product?.listPrice.formatted?.withoutTax} </span>
                                        ) : null}
                                  ...
                                  <div className="justify-end">
                                    <span className="flex flex-col font-semibold text-black"> {translate('common.label.qtyText')} {product?.qty} </span>
                                  </div>
                                </div>
                              </div>
                            </div>
                ...
                <div className="px-4 mt-0">
                  <PromotionInput deviceInfo={deviceInfo} basketPromos={basketPromos} items={data} getBasketPromoses={getBasketPromos} />
                </div>
                <div className="w-full px-4">
                  <dl className="mt-2 space-y-2 sm:space-y-2">
                    <div className={data?.deliveryPlans?.length > 0 ? 'py-3 border-b border-gray-200' : ''} >
                          ...
                          {data?.estimatedDeliveryDate && (
                            <>
                              ...
                              <dd className="block text-black font-18">
                                {isIncludeVAT ? data?.shippingCharge?.raw?.withTax == 0 ? 'FREE' : data?.shippingCharge?.formatted?.withTax || EmptyString : data?.shippingCharge?.raw?.withoutTax == 0 ? 'FREE' : data?.shippingCharge?.formatted?.withoutTax || EmptyString}
                              </dd>
                            </>
                          )}
                        </div>
                      ...
                      {groupedPromotions?.autoAppliedPromos?.length > 0 && (
                        <div className="flex items-end justify-between pt-2 mb-2 sm:pt-1">
                          <dt className="flex flex-col items-start text-sm text-black">
                            ...
                            {groupedPromotions?.autoAppliedPromos?.map((promo: any, idx: number) => (
                              <span key={idx} className="block mt-1 font-18"> {promo?.name} </span>
                            ))}
                          </dt>
                          {groupedPromotions?.autoAppliedPromos?.map((promo: any, idx: number) => promo?.discountAmt?.raw?.withTax > 0 && (
                            <dd key={idx} className="block text-black font-18" > - {isIncludeVAT ? promo?.discountAmt?.formatted?.withTax : promo?.discountAmt?.formatted?.withoutTax} </dd>
                          ))}
                        </div>
                      )}
                      ...
                      {groupedPromotions?.appliedPromos?.length > 0 && (
                        <div className="flex items-end justify-between pt-2 mb-2 sm:pt-1">
                          <dt className="flex flex-col items-start text-sm text-black">
                            ...
                            {groupedPromotions?.appliedPromos?.map((promo: any, idx: number) => (
                              <span key={idx} className="block mt-1 font-18"> {promo?.promoCode} </span>
                            ))}
                          </dt>
                          {groupedPromotions?.appliedPromos?.map((promo: any, idx: number) => (
                            <dd key={idx} className="block text-black font-18" > - {isIncludeVAT ? promo?.discountAmt?.formatted?.withTax : promo?.discountAmt?.formatted?.withoutTax} </dd>
                    ...
                    {cartItems?.deliveryPlans?.length > 0 && (
                      <div className="flex items-center justify-between pt-2 sm:pt-1">
                        ...
                        <dd className="font-semibold text-black text-md">
                          {isIncludeVAT ? cartItems?.shippingCharge?.raw?.withTax == 0 ? translate('label.orderSummary.freeText') : cartItems?.shippingCharge?.formatted?.withTax || EmptyString : cartItems?.shippingCharge?.raw?.withoutTax == 0 ? translate('label.orderSummary.freeText') : cartItems?.shippingCharge?.formatted?.withoutTax || EmptyString}
                        </dd>
                      </div>
                    )}
                      ...
                      <div className="flex flex-row sm:px-0">
                        {referralAvailable && !referralInfo && (
                          <div className="flex flex-col w-full max-w-lg my-10 2xl:justify-center xl:items-center px-9">
                            <h2 className="mx-2 text-[30px] text-center"> {translate('label.checkout.searchFriendByReferralCodeText')} </h2>
                            <p className="px-8 text-[18px] text-center"> {translate('label.checkout.friendSignupConfirmationText')} </p>
                            <input type="text" placeholder={translate('label.checkout.enterReferralCodeText')} className="px-5 w-full my-2 py-3 border-[1px] border-gray-500" onChange={handleInputChange} />
                            {error && (
                              <p className="text-sm text-red-700">{error}</p>
                            )}
                            <Button className="my-3" onClick={() => { handleReferralSearch() }} >
                              {isLoading ? <LoadingDots /> : translate('label.myAccount.findReferralBtnText')}
                            </Button>
                          </div>
                        )}
                        {referralInfo && (
                          <div className={classNames('my-10 flex w-full flex-col justify-center items-center')} >
                            <h2 className="px-5 text-center"> {translate('label.checkout.friendFoundConfirmationText')} </h2>
                            <div className="py-2 flex flex-row border-[1px] my-5 items-center justify-center border-gray-600">
                              <p className="px-3 !mt-0 text-center font-bold "> {translate('label.checkout.voucherCodeText')}: {referralInfo?.voucherCode} </p>
                            ...
                            <p className="px-5 font-bold text-center"> Offer: {referralInfo?.promoName} </p>
                            <p className="font-bold">
                              {translate('label.checkout.validityText')}:{' '}
                              {`${formatFromToDates(referralInfo?.validFrom, referralInfo?.validTo)}`}
                            </p>
                            <p className="px-12 text-center"> {translate('common.label.availGiftText')}</p>
                          </div>
                        )}
                        <div className="flex w-full xl:h-[439px] 2xl:h-auto 2xl:object-none xl:object-cover">
                          <img src={'/assets/images/refer-a-friend.jpg'} alt="banner" height={700} width={480} className="object-cover" ></img>
                        </div>
                      </div>
                    </div>
                  </div>
