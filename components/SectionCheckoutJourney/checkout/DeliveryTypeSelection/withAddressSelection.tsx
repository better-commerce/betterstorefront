  ...
  const renderAddressFormUI = useMemo(() => {
    return () => {
      ...
      if (deliveryTypeMethod?.type?.includes(DeliveryType.STANDARD_DELIVERY) || deliveryTypeMethod?.type?.includes(DeliveryType.EXPRESS_DELIVERY)) {
        ...
        return <BillingAddressForm shippingCountries={shippingCountries} billingCountries={billingCountries} searchAddressByPostcode={searchAddressByPostcode} onSubmit={onSubmit} useSameForBilling={false} shouldDisplayEmail={false} currentStep={currentStep} appConfig={appConfig} featureToggle={featureToggle} />
      } else {
        return <></>
      }
    }
