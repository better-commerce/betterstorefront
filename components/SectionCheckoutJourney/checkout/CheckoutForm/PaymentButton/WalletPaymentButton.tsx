...
export class WalletPaymentButton extends BasePaymentButton {
  ...
  private async onPay(paymentMethod: any, basketOrderInfo: any, uiContext: any, dispatchState: Function) {
    ...
    if (uiContext?.user?.userId) {
      ...
      const amountToBePaid = this.isFullPayment() ? (basketOrderInfo?.basket?.isPartialPayment ? basketOrderInfo?.basket?.partialPayableAmount?.raw : basketOrderInfo?.basket?.grandTotal?.raw?.withTax) : (this.props?.partialAmount || 0)
      if (amountToBePaid <= 0) {
