...
const Coupon = (props: IPromotionInputProps) => {
  const { basketPromos, getBasketPromoses = () => { }, isShowCoupon, setShowCoupon, isCouponApplied, setCouponApplied, appliedBasketPromo, setAppliedBasketPromo, multiPromo, setMultipromo, value, setValue, error, setError } = props
  ...
  const handleMultiPromo = (promo: any) => {
    ...
    if (promo?.promoType == 2 || promo?.promoType == 4 || promo?.promoType == 9) {
  ...
  const multiSelectPromoAvailable = basketPromos?.availablePromotions?.filter((x: any) => x?.useWithOtherDiscountCode == 3)
  const multiSelectPromoApplicable = basketPromos?.applicablePromotions?.filter((x: any) => x?.useWithOtherDiscountCode == 3)
  const singleSelectPromo = basketPromos?.availablePromotions?.filter((x: any) => x?.useWithOtherDiscountCode == 1) || basketPromos?.availablePromotions?.filter((x: any) => x?.useWithOtherDiscountCode == 2)
  const applied = cartItems?.promotionsApplied?.find((x: any) => x.promoCode)

  return (
    <>
      <Transition.Root show={isShowCoupon} as={Fragment}>
        <Dialog aria-label="list-coupons" as="div" className="relative z-9999" onClose={setShowCoupon} >
                  ...
                  <Dialog.Panel className="w-screen max-w-md pointer-events-auto width-cart">
                    <div className="relative flex flex-col h-full bg-white shadow-xl z-99">
                      <div className="sticky top-0 z-10 px-4 py-4 border-b border-gray-200 sm:px-6 left-1">
                        <div className="flex">
                          <button type="button" className="mr-2 text-black rounded-md outline-none hover:text-gray-500" onClick={() => setShowCoupon(false)} >
                            <span className="sr-only">{translate('common.label.closePanelText')}</span>
                            <ArrowLeftIcon className="relative top-0 w-5 h-5" aria-hidden="true" />
                          </button>
                          <h3 className="font-display text-md sm:text-lg dark:text-black"> {translate('label.basket.applyCouponsText')} </h3>
                        </div>
                          ...
                          {cartItems?.promotionsApplied?.length ? cartItems?.promotionsApplied.map((promo: any, key: number) => {
                            return (
                              <div className="flex justify-between gap-4 pt-2 my-1" key={key} >
                                <div className="flex">
                                  <h5 className="font-medium uppercase text-primary dark:text-black text-14 xs-text-14"> {' '} {promo.promoCode} </h5>
                                </div>
                                  ...
                                  <h5 className="font-medium uppercase text-14 xs-text-14 text-emerald-600">
                                    ...
                                    {!promo?.autoApply && (
                                      <a href="javascript: void(0);">
                                        <span className="relative ml-2 sprite-icon cross-icon top-0.5" onClick={() => handleSubmit('remove', promo.promoCode)} ></span>
                                      </a>
                                    )}
                                  </h5>
                                </div>
                      ...
                      <div className="p-6 overflow-y-auto">
                        <div className="flex flex-col w-full">
                          {multiSelectPromoApplicable?.length > 0 && (
                            <div className="flex flex-col mb-4">
                              <div className="grid grid-cols-1">
                                ...
                                {multiSelectPromoApplicable?.map((promo: any, idx: number) => {
                                  return (
                                    <div key={idx} className="mt-3 border border-gray-200" >
                                      <div className="grid grid-cols-12">
                                        <div className="relative col-span-1 text-white bg-black coupon-code-panel coupon-cross">
                                          <h3 className="font-semibold text-white uppercase coupon-text-rotate"> {promo?.additionalInfo7} </h3>
                                        </div>
                                        <div className="col-span-11 p-4 coupon-code-data">
                                          <div className="flex justify-between">
                                            <h3 className="mb-1 text-sm font-semibold text-black uppercase"> {promo?.code} </h3>
                                            <div className="font-medium text-left text-black text-md">
                                              <input type="checkbox" className="custom-checkbox-promo" checked={multiPromo?.includes(promo)} onChange={() => handleMultiPromo(promo)} value={promo?.code} name="applyPromo" />
                                            </div>
                                          </div>
                                          <div className="flex flex-col">
                                            <p className="text-xs font-medium text-emerald-500"> {promo?.name} </p>
                                          </div>
                                          <div className="flex flex-col pt-4 mt-4 border-t border-gray-200 border-dotted px">
                                            <p className="text-xs font-normal text-gray-400"> {promo?.additionalInfo6} </p>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                            ...
                            <div className="flex flex-col">
                              <div className="grid grid-cols-1">
                                ...
                                {singleSelectPromo?.map((promo: any, idx: number) => {
                                  return (promo?.promoType != 22 && (promo?.useWithOtherDiscountCode == 1 || promo?.useWithOtherDiscountCode == 2 || promo?.useWithOtherDiscountCode == 4) && (
                                    <div key={idx} className="mt-3 border border-gray-200" >
                                      <div className="grid grid-cols-12">
                                        <div className={`relative col-span-1 text-white coupon-cross ${applied?.promoCode == promo?.code ? `bg-black coupon-code-panel` : 'bg-black coupon-code-panel'}`} >
                                          <h3 className="text-white uppercase font-display coupon-text-rotate"> {promo?.additionalInfo7} </h3>
                                        </div>
                                        <div className="col-span-11 p-4 coupon-code-data">
                                          <div className="flex justify-between">
                                            <h3 className="mb-1 text-black uppercase text-md font-display"> {promo?.code} </h3>
                                            {applied?.promoCode == promo?.code ? (
                                              <button type="button" className="text-md font-display text-emerald-500" >
                                                {translate('label.basket.appliedText')}
                                              </button>
                                            ) : (
                                              <button type="button" className="text-black text-md font-display" onClick={(ev: any) => applyCoupon(promo, ev)} >
                                                {translate('label.basket.appliedText')}
                                              </button>
                                            )}
                                          </div>
                                          <div className="flex flex-col">
                                            <p className="font-medium text-ms text-emerald-500"> {promo?.name} </p>
                                          </div>
                                          <div className="flex flex-col pt-4 mt-4 border-t border-gray-200 border-dotted px">
                                            <p className="font-normal text-gray-400 text-ms"> {promo?.additionalInfo6} </p>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                        ...
                        <div className='flex flex-col'>
                          {multiSelectPromoAvailable?.length > 0 && (
                            <div className="flex flex-col mb-4">
                              <div className="grid grid-cols-1">
                                <h4 className="text-lg font-semibold text-dark-brown dark:text-black">
                                  {translate('label.basket.moreOffersText')}
                                </h4>
                                {multiSelectPromoAvailable?.map((promo: any, idx: number) => {
                                  return (
                                    <div key={idx} className="mt-3 border border-gray-200" >
                                      <div className="grid grid-cols-12">
                                        <div className="relative col-span-1 text-white bg-gray-300 coupon-code-panel coupon-cross">
                                          <h3 className="font-semibold text-white uppercase coupon-text-rotate"> {promo?.additionalInfo7} </h3>
                                        </div>
                                        <div className="col-span-11 p-4 coupon-code-data">
                                          <div className="flex justify-between">
                                            <h3 className="mb-1 text-sm font-semibold text-gray-400 uppercase"> {promo?.code} </h3>
                                          </div>
                                          <div className="flex flex-col">
                                            <p className="text-xs font-medium text-gray-400"> {promo?.name} </p>
                                          </div>
                                          <div className="flex flex-col pt-4 mt-4 border-t border-gray-200 border-dotted px">
                                            <p className="text-xs font-normal text-gray-400"> {promo?.croMessage} </p>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                      ...
                      {multiPromo?.length > 0 && (
                        <div className="sticky bottom-0 z-10 flex flex-col w-full px-6 py-4 bg-white">
                          <button onClick={() => applyMultiPromo()} type="button" className="w-full btn btn-primary button-primary" >
                      ...
                      <div className="relative w-full h-auto max-w-md mx-auto bg-white center-fix-panel top-2/4 -translate-y-2/4">
                        <div className="sticky top-0 z-10 px-4 py-4 border-b border-gray-200 sm:px-6 left-1">
                          <div className="flex justify-between">
                            ...
                            <button type="button" className="mr-2 text-black rounded-md outline-none hover:text-gray-500" onClick={() => setCouponApplied(false)} >
                              <span className="sr-only">{translate('common.label.closePanelText')}</span>
                              <XMarkIcon className="relative top-0 w-6 h-6" aria-hidden="true" />
                            </button>
                          </div>
                        </div>
                        <div className="flex flex-col px-6">
                          <div className="mt-3 border border-gray-200">
                            <div className="grid grid-cols-12">
                              ...
                              {cartItems?.promotionsApplied?.filter((x: any) => x?.promoType == 9)?.length > 0 || cartItems?.promotionsApplied?.filter((x: any) => x?.promoType == 2)?.length > 0 || cartItems?.promotionsApplied?.filter((x: any) => x?.promoType == 4)?.length > 0 || cartItems?.promotionsApplied?.length == 0 ? (
                                <div className="col-span-11 p-4 coupon-code-data">
                                  <div className="flex justify-between">
                        ...
                        <div className="flex flex-col px-6 py-4">
                          <button type="button" className="w-full text-white bg-black border border-black btn-basic-property hover:text-white hover:bg-gray-800" onClick={() => setCouponApplied(false)} >
