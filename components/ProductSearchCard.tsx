...
const ProductSearchCard: FC<ProductCardProps> = ({ className = "", data, isLiked, deviceInfo, maxBasketItemsCount, key, featureToggle, defaultDisplayMembership }) => {
...
const cashbackAmount = data?.attributes?.find(((item: any) => item?.key == "cashback.amount"))?.value
  ...
  return (
    <>
      <div key={key} className={cn(`${featureToggle?.features?.enableForPCSite ? 'border border-gray-200 p-0 rounded' : 'border-prod-card'} grid grid-cols-12 gap-3 ${product?.compared ? featureToggle?.features?.enableForPCSite ? '!border !border-sky-600' : '!border !border-orange-600' : ''} ${className}`, { 'height-full': isComparedEnabled, 'height-full border-amber-400 rounded-t-3xl rounded-b-2xl border-2': product?.compared, })}>
        <div className={`${featureToggle?.features?.enableForPCSite ? '' : 'bg-slate-50 dark:bg-slate-300 rounded-3xl'} col-span-3 relative flex-shrink-0 overflow-hidden z-1 group rounded-green product-card__image-container`}>
          <ButtonLink isComparedEnabled={isComparedEnabled} href={sanitizeRelativeUrl(`/${data?.slug || data?.link}`)} itemPrice={itemPrice} productName={data.name} onClick={handleSetCompareProduct}>
            <div className="flex w-full py-2 product-card__image">
        ...
        <div className='col-span-9 py-2 pl-5 border-l border-gray-200'>
          <ButtonLink isComparedEnabled={isComparedEnabled} href={sanitizeRelativeUrl(`/${data?.slug || data?.link}`)} itemPrice={itemPrice} productName={data?.name} onClick={handleSetCompareProduct}>
            <div className={`${featureToggle?.features?.enableForPCSite ? 'px-0 pt-0 pb-1 ' : 'px-2.5 pt-5 pb-2.5 '} product-card__information`}>
              <div className=''>
