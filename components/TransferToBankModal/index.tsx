...
export default function TransferModal({
  ...
  const handleSubmit = (values: any) => {
    const selectedBankDetails: any = customerBankList.find(
      (item: any) => item?.data?.customerBankId === values.accountDetails
    )
    if (!selectedBankDetails) return
    const payload = {
      walletId: selectedBankDetails?.data?.walletId,
      amount: values.amount,
      currency: walletDetail?.currency,
      bankName: selectedBankDetails?.data?.bankName,
      accountNumber: selectedBankDetails?.data?.accountNumber,
      sortCode: selectedBankDetails?.data?.sortCode,
    }
  ...
  const getCustomerBankList = async (walletId: any) => {
    try {
      const config: AxiosRequestConfig = {
        url: NEXT_GET_CUSOMER_BANK_LIST,
        method: RequestMethod.POST,
        data: { walletId },
      }
      const { data }: any = await callApi(config)
      setCustomerBankList(data?.data?.items)
      setCustomerBankDropdown(
        data?.data?.items?.map((item: any) => ({
          itemText: `${item?.data?.accountNumber} (${item?.data?.bankName})`,
          itemValue: item?.data?.customerBankId,
        })) || []
      )
    } catch (error) {
      logError(error)
                      ...
                      return (
                        <Form>
                          ...
                          <div className="mt-6 flex gap-4 justify-end">
                            ...
                            <button
                              type="submit"
                              className="inline-flex justify-center px-4 py-2 text-sm font-medium text-white ttnc-ButtonPrimary border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
                            >
                              Save
                            </button>
                          </div>
