                        ...
                        <div className="flex">
                          ...
                          <div className="w-64">
                            ...
                            <div
                              ...
                              className={`w-full px-3 py-3 capitalize flex justify-between items-center border-link-bottom border-top-link text-sm font-bold text-black text-left transition-colors ${selectedCategory === "featured"
                            ...
                            >
                              <span>Featured</span>
                              <ChevronRightIcon
                                className={`inline-block w-4 h-4 ml-2 ${selectedCategory === "featured" || hoveredTab === "featured"
                                  ? 'block'
                                  : 'hidden'
                                  }`}
                              />
                                ...
                                {grp?.widgets?.map((navItem: any, kdx: number) => {
                                  return (
                                    navItem?.navItems?.length > 0 && navItem?.blockUrl === "" ?
                                      ...
                                      : <Link href={navItem?.blockUrl != "" ? sanitizeRelativeUrl(`/${navItem?.blockUrl}`) : '#'} onMouseLeave={handleTabLeave} onClick={handleLinkClick} passHref className={`w-full capitalize px-3 py-3 flex justify-between items-center border-link-bottom text-sm font-bold text-black text-left transition-colors hover:bg-[#F5F5F5]`}><span>{navItem?.boxTitle?.toLowerCase()}</span></Link>
                                  )
                                })}
                              </div>
                            ))}
                            ...
                            {selectedCategory === "featured" && (
                              <div className="w-full mid-mega-container">
                                ...
                                {getWidgets(
                                  ...
                                  <div
                                    className="grid grid-flow-col gap-6 auto-cols-max place-content-center"
                                          ...
                                          {navItem?.navItems.sort((a: any, b: any) => a?.caption?.localeCompare(b?.caption)).map((item: any, idx: number) => (
                                            <li key={`category-link-${idx}`}>
                                              <Link
                                                ...
                                                className="text-sm font-semibold text-black capitalize hover:text-black hover:underline"
                                                onClick={handleLinkClick}
                                                prefetch={false}
                                              >
                                                {item?.caption?.toLowerCase()}
