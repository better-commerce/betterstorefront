...
import SubmittingLoader from './SubmitLoader'
...
const SubmitButton = (props: ISubmitButtonProps) => {
  ...
  const eventInitiated =
    submitState &&
    submitState?.isSubmitting &&
    submitState?.submitSource === source &&
    (!submitState?.id ||
      (submitState && submitState?.id && submitState?.id === (loaderId ?? '')))
        ...
        <button
          ...
          className={`${cssClass} ${
            disabled
              ...
              : submittingClassName(submitState, source)
          }`}
          disabled={disabled}
          onClick={(ev: any) => {
            if (eventInitiated) {
