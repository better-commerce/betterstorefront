...
const SearchBar: FC<React.PropsWithChildren<ISearchBarProps>> = ({ id = 'search', onClick, keywords, maxBasketItemsCount, deviceInfo, searchDefaultSortBy}) => {
  ...
  const defaultSearch = (
    <>
      <div className="relative search-icon-div items-center justify-center w-10 h-10 rounded-full group theme-search-bar theme-search-bar-icon lg:top-0 top-header-search lg:flex sm:w-12 sm:h-12 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-100 focus:outline-none" onClick={onClick}>
