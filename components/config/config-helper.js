...
export function getBrandField() {
  // If no title field configuration has been provided, we attempt
  // to use a "title" field, if one exists
  return getConfig().brandField || 'Brands'
}
...
export function getResultBrand(result) {
  const brandField = getBrandField()

  return result.getSnippet(brandField)
}
...
export function buildSearchOptionsFromConfig() {
  ...
  if (config.brandField) {
    resultFields[config.brandField] = {
      raw: {},
      snippet: {
        size: 100,
        fallback: true,
