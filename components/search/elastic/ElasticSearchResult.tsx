...
const CustomResultView = ({ result }: any, includeOutOfStockProduct: boolean, maxBasketItemsCount: any) => {
  const {
    deviceInfo,
    basketId,
    user,
        ...
        const item = await cartHandler()?.addToCart(
          {
            ...
            qty: quantity,
            manualUnitPrice: result?.[getElasticSearchPriceColumn(isIncludeVAT)]?.raw,
  ...
  return (
    <>
      {(!includeOutOfStockProduct ? matchStrings(result?.availability?.raw, 'In stock', true) : true) && <li className="relative grid grid-cols-12 p-4 mb-0 bg-white border border-gray-200 rounded-md sm:grid-cols-1 hover:border-orange-500 group-hover:border-gray-700 snap-start hover:text-blue-600 group">
        ...
        <div className="col-span-8 sm:col-span-12">
          <h5 className="mb-2 font-semibold text-black capitalize font-18">
            {priceFormat(result?.[getElasticSearchPriceColumn(isIncludeVAT)]?.raw)}
...
function ElasticSearchResult({
  ...
  const [includeOutOfStockProduct, setIncludeOutOfStockProduct] = useState(false)
  return (
                ...
                <>
                  {wasSearched && (
                    <Results
                      ...
                      resultView={(result: any) => { return CustomResultView(result, includeOutOfStockProduct, maxBasketItemsCount) }}
                ...
                <React.Fragment>
                  <div className="grid items-center w-full grid-cols-12 sm:gap-4 sm:grid-cols-12">
                    <div className="col-span-4 mb-2 sm:col-span-6">
                      {
                        wasSearched && <>
                          <h3 className='font-semibold text-black uppercase font-18 sm:mb-0'>Product Suggestions</h3>
                          <PagingInfo />
                        </>
                      }
                    </div>
