...
export default function FinanceCalculator({ product, loanCalculatorOpen }: { product: any, loanCalculatorOpen: boolean }) {
    ...
    const [loanCalcParams, setLoanCalcParams] = useState<{ loanProductId: string, depositAmountPerc: number, numberOfInstallments: number }>({ loanProductId: "", depositAmountPerc: 0, numberOfInstallments: 0 })
    ...
    useEffect(() => {
        const asyncHandler = async () => {
            ...
            await getLoanDetails({ loanProductId: sortedProductRates[0]?.Id, depositAmountPerc: DEPOSIT_AMOUNT_OPTIONS[0]?.value, numberOfInstallments: sortedProductRates[0]?.Term })
    ...
    useEffect(() => {
        ...
        const asyncHandler = async (params: { loanProductId: string, depositAmountPerc: number, numberOfInstallments: number }) => {
            if (loanCalcParams?.loanProductId !== params?.loanProductId) {
                await getLoanDetails(loanCalcParams)
                hideOverlayLoaderState()
                debugger
            } else {
        ...
        if (loanCalcParams?.loanProductId && loanCalcParams?.loanProductId !== emptyGuid && loanCalcParams?.depositAmountPerc && loanCalcParams?.numberOfInstallments) {
            setOverlayLoaderState({ visible: true, message: translate('common.message.loaderLoadingText'), })
            asyncHandler(loanCalcParams)
        }
    }, [loanCalcParams])
    ...
    return (
        ...
        <div className="px-4 py-3">
            ...
            <div className="relative">
                <select className="w-full py-2 pl-3 pr-8 border border-gray-300 rounded bg-white appearance-none text-gray-700 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" onChange={(ev: any) => reCalcFinance({ loanProductId: "", depositAmountPerc: 0, numberOfInstallments: parseInt(ev?.target?.value) })}>
                    {Array.from({ length: numberOfInstallments }, (_, i) => i + 1).map((option: any) => (
