...
export default function FinanceCalculator({ product, loanCalculatorOpen }: { product: any, loanCalculatorOpen: boolean }) {
    ...
    const [loanCalcParams, setLoanCalcParams] = useState<{ loanProductId: string, depositAmountPerc: number }>({ loanProductId: "", depositAmountPerc: DEPOSIT_AMOUNT_OPTIONS[0]?.value })
    const [loanDetails, setLoanDetails] = useState<any>({})
    const loanProductRef = useRef<any>(null)
    ...
    const reCalcFinance = useCallback((loanCalcParams: { loanProductId: string, depositAmountPerc: number }) => {
    ...
    const getLoanDetails = useCallback(async (params: { loanProductId: string, depositAmountPerc: number }) => {
    ...
    useEffect(() => {
        const asyncHandler = async () => {
            ...
            const rateCard = await getLoanDetails({ loanProductId: sortedProductRates[0]?.Id, depositAmountPerc: DEPOSIT_AMOUNT_OPTIONS[0]?.value })
    ...
    useEffect(() => {
        const fetchLoanDetails = async () => {
            const loanProductId = loanProductRef?.current?.value;
            if (loanProductId && loanProductId !== emptyGuid) {
                setOverlayLoaderState({ visible: true, message: translate('common.message.loaderLoadingText') });
                
                try {
                    const rateCard = await getLoanDetails({ ...loanCalcParams, loanProductId });
    ...
    return (
        <div className="w-full">
            <div className="w-full sm:w-[75%] bg-white rounded-lg border border-gray-300 shadow-sm overflow-hidden">
            {/* Header Section */}
            ...
            <div className="px-4 py-3">
                ...
                <div className="mb-4">
                    ...
                    <div className="relative">
                        <select ref={loanProductRef} className="w-full py-2 pl-3 pr-8 border border-gray-300 rounded bg-white appearance-none text-gray-700 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500" onChange={(ev: any) => reCalcFinance({ loanProductId: ev?.target?.value, depositAmountPerc: 0 })}>
