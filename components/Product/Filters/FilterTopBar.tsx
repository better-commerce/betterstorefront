                  ...
                  <div key={`applied-filter-top-bar-${idx}`} className="flex justify-center m-1 text-gray-600" >
                    {appliedFilter?.name && (
                      <span className="flex px-2 py-1 font-medium text-gray-600 border font-14 border-slate-300 bg-slate-50 rounded-2xl">
                        {secondValue}
                        <span onClick={() => removeFilter(appliedFilter)} className="relative flex items-center justify-center flex-shrink-0 w-4 h-4 ml-3 text-white bg-red-500 rounded-full cursor-pointer top-0.5">
