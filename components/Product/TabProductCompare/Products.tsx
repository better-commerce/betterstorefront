...
const Products: FC<React.PropsWithChildren<Props & IExtraProps>> = ({
  ...
  return (
    <>
      <div
        className="flex flex-col bg-white prod-group md:pb-0 pb-14 lg:pb-14"
        key={product.id}
      >
        ...
        <Link passHref href={`/${currentProductData.link}`} title={`${product.name} \t ${itemPrice}`} >
          <div className="text-base mt-2 font-semibold transition-colors min-h-[60px] nc-ProductCard__title text-left  ">
            {product?.name}
          </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 flex flex-col add-btn-plp">
          {isOutOfStock(product) ? (
            ...
            <Button className="text-sm font-medium rounded-md cart-btn-plp" title={buttonConfig.title} action={buttonConfig.action} type="button" buttonType={buttonConfig.buttonType || 'cart'} />
          )}
        </div>
      </div>
