    ...
    <div className="grid grid-cols-5 sm:grid-cols-7 gap-2 mt-2.5">
      {defaultItems.map((item: any, idx: any) => (
        <RadioGroup.Option key={idx} value={item.fieldValue} title={item.fieldLabel} onClick={() => { setSizeInit('false') }} style={{ backgroundColor: item.fieldValue }}
          className={cn(' pdp-color-swatch-item relative z-1 rounded-xl h-10 border border-gray-200 items-center attr-color justify-center cursor-pointer outline-none ring-gray-600 ring-inset-1 hover:ring-1', { 'ring-1 z-1': selectedValue === item.fieldValue, })} />
      ))}
  ...
  return (
    <>
      <div className="flex flex-col justify-start gap-1 mt-2">
        <h4 className="text-gray-700 font-14 dark:text-white font-label-14">{label} :
          {validation && <span className='relative inline-block w-5 h-5 ml-1 border border-gray-200 rounded-full top-1' style={{ background: colorName }}></span>}
        </h4>
      </div>
