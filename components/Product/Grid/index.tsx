    ...
    <>
      {IS_INFINITE_SCROLL && (
        <InfiniteScroll
          ...
          component={
            <div className={`p-[1px] border-gray-100 gap-x-4 gap-y-4 grid grid-cols-1 sm:mx-0 md:grid-cols-2 px-3 sm:px-2 ${products.results.length < 4 ? `lg:grid-cols-3` : featureToggle?.features?.enableHorizontalFilter ? 'lg:grid-cols-4' : featureToggle?.features?.enablePCHome ? 'lg:grid-cols-3' : 'lg:grid-cols-4'}`} >
        ...
        <>
          <div className={`p-[1px] border-gray-100 gap-x-4 gap-y-4 grid grid-cols-1 sm:mx-0 md:grid-cols-2 px-3 sm:px-2 ${products.results.length < 4
            ...
            : featureToggle?.features?.enableHorizontalFilter
              ...
              : featureToggle?.features?.enablePCHome ? 'lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-4' : 'lg:grid-cols-3 xl:grid-cols-3 2xl:grid-cols-3'
            }`}>
            {!products?.results?.length && rangeMap(12, (i) => (
              <div key={i} className="mx-auto mt-20 rounded-md shadow-md w-60 h-72">
                <div className="flex flex-row items-center justify-center h-full space-x-5 animate-pulse">
