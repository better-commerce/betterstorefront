...
const DefaultButton: FC<React.PropsWithChildren<Props>> = ({
  ...
  return formId ? (
    <button
      type="submit"
      form={formId}
      className={`xs:max-w-xs flex-1 btn-green-new ${bgColor} border border-transparent flex items-center justify-center hover:${hoverBgColor} focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-50 focus:${focusRingColor} sm:w-full ${className} btn btn-c btn-primary`}
    ...
    >
      {isLoading ? (<LoadingDots />) : title == 'Add' ? (
        <>
          <PlusCircleIcon className="inline-block w-5 h-5 relative -top-0.5" />{' '}{title}
        </>
      ) : (
        title
      )}
