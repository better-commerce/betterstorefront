                ...
                matchStrings(item?.type, "checkbox", true) && (
                    <>
                        <Field
                            ...
                            checked={
                                item?.checked ? item?.checked(context) : context?.values[item?.name]
                            }
                            disabled={context.initialValues && item?.disabled ? true : false}
                            required={item?.required}
                        />
                                    ...
                                    <>
                                        {
                                            item?.options?.map((option: any, idx: number) => (
                                                <a key={idx}
                                                    ...
                                                    className={idx == item?.options?.length - 1
                                                        ? classNames(item?.lastOptionClassName, idx === item?.activeOptionIndex ? " active" : "")
                                                        : classNames(item?.optionClassName, idx === item?.activeOptionIndex ? " active" : "")}
                                                    onClick={(e: any) => {
                                                        setFieldValue(item?.name, option?.value);
                                                    }}
                                                >
                                                    <input className="main-custom-check" id={option?.label} name="address-type" type="radio" defaultChecked={((idx === item?.activeOptionIndex) || context?.values[item?.name] === option?.value) ? true : false} />
                                                    <label className="text-sm main-custom-check-label" htmlFor={option?.label} title="Smooth Payments">
                                                        {option?.label}
                                                    </label>
                                                </a>
                                            ))
                                        ...
                                        {item?.required && (
                                            getErrorLabel(context, item, true)
                                        )}
                                    </>
                                );
                            }}
