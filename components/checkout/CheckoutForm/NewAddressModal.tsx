...
export const NEW_ADDRESS_FORM_SCHEMA = Yup.object().shape({
    ...
    pinCode: Yup.string().required(Messages.Validations.AddNewAddress["PIN_CODE_REQUIRED"]).min(3)
...
export const DEFAULT_ADDRESS_VALUES = {
    ...
    name: "",
    mobileNumber: "",
    categoryName: findByFieldName(NEW_ADDRESS_FORM_FIELDS, "categoryName")?.options?.length
        ? findByFieldName(NEW_ADDRESS_FORM_FIELDS, "categoryName")?.options[0]?.value
        : "",
    otherAddressType: "",
    whtsappUpdated: true,
};
                                        ...
                                        <div className="relative z-50 flex flex-col h-full bg-white shadow-xl">
                                            <div className="z-10 px-4 py-6 pb-2 border-b sm:px-6 left-1 top-1">
                                                <div className='flex justify-between pb-2 mb-0'>
                                                    <h3 className="font-bold text-black text-20 dark:text-black"><a onClick={onCloseModal} className="inline-block cursor-pointer sm:hidden" >
                                                        <i className='mr-2 sprite-icon sprite-left-arrow'></i></a>
                                            ...
                                            <div className="p-0 px-2 py-2 overflow-y-auto sm:p-0 sm:px-2">
                                                ...
                                                <NewAddressForm
                                                    ...
                                                    defaultValues={selectedAddress && selectedAddress?.id
                                                        ? {
                                                            ...
                                                            categoryName: findByFieldName(NEW_ADDRESS_FORM_FIELDS, "categoryName")?.options?.filter((x: any) => x?.label != "Other")?.map((x: any) => x.label)?.includes(selectedAddress?.label) ? selectedAddress?.label : "Other",
                                                            otherAddressType: !findByFieldName(NEW_ADDRESS_FORM_FIELDS, "categoryName")?.options?.filter((x: any) => x?.label != "Other")?.map((x: any) => x.label)?.includes(selectedAddress?.label) ? selectedAddress?.label : "",
                                                            whtsappUpdated: selectedAddress?.isConsentSelected,
                                                        }
