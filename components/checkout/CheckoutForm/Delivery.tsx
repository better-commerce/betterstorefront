                ...
                return (
                  <div key={idx} className="flex flex-col my-1">
                    <li
                      ...
                      className={`${shippingMethod.id === item.id ? 'border-indigo-500 border-2' : 'border'
                        }  pointer py-5 px-5 flex justify-between rounded-md cursor-pointer flex-row`}
                    >
                      <div className='pr-10'>
                        <h3 className="font-bold strong-space" dangerouslySetInnerHTML={{ __html: item.displayName || '' }}></h3>
                        <p className="text-xs py-2 strong-space text-gray-500" dangerouslySetInnerHTML={{ __html: item.description || '' }}></p>
                      </div>
                      <div className="flex flex-row justify-center items-center">
                        <h3 className='font-bold'>{item.price.formatted.withTax}</h3>
