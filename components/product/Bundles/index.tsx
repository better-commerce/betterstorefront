...
export default function Bundles({ price = '', products = [], productBundleUpdate = () => { } }: any) {
  ...
  return (
    <section aria-labelledby="bundle" className="px-4 pt-4 border-t border-gray-200 sm:px-0">
      <h2 id="bundle" className="text-3xl font-bold text-center text-gray-900">{YOUR_BUNDLE_INCLUDE}</h2>
      ...
      <div className="flex flex-col justify-end p-3 border border-gray-100 rounded-md bg-gray-50 items-right">
        <p className="font-semibold text-right text-black text-md flext-col align-right item-right">Bundle Price</p>
        <p className="flex flex-col pb-2 text-3xl font-bold text-right text-gray-900 align-right item-right">{price}</p>
      </div>
      {productData && (<BundleCard closeModal={() => handleProduct(null)} productData={productData} />)}
    </section>
  )
}