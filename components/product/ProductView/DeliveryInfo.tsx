...
export const DELIVERY_FORM_FIELDS = [
  {
    ...
    showLabel: false,
    label: "Pincode",
    className: "px-3 py-4 text-sm font-normal text-black border bg-zinc-100 border-zinc-100 placeholder:text-gray-400 hover:border-gray-900",
...
export default function DeliveryInfo({ product, grpData }: any) {
  ...
  if (grpData["product.returnexchangeeligibility"]?.length > 0) {
    const data = grpData["product.returnexchangeeligibility"].find((x: any) => x.value);
  ...
  return (
    <>
      ...
      <section aria-labelledby="details-heading" className="mt-0 border-gray-200 sm:border-t sm:mt-2">
        <div className="flex flex-col px-4 pt-0 pb-0 mt-0 sm:pb-6 sm:pt-2 sm:mt-2 sm:px-0">
          ...
          <p className='font-normal text-14 text-brown-light'>For orders above ₹600, Usually delivered in 2-5 days</p>
      ...
      <ProductWarranty isWarranty={isWarranty} setWarranty={setWarranty} replaceValue={replaceValue} />
      <ProductReturn isReturn={isReturn} setReturn={setReturn} data={grpData["product.returneligibilitydays"]} />
    </>
  )
}