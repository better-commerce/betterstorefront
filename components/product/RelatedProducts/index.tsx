...
export default function RelatedProducts({ relatedProducts = [], relatedProductList = [], }: any) {
  ...
  const addToCart = (product: any) => {
    const asyncAddToCart = async () => {
      const item = await cartHandler().addToCart({
        basketId: basketId, productId: product.recordId, qty: 1, manualUnitPrice: product.price.raw.withTax,
        stockCode: product.stockCode, userId: user.userId, isAssociated: user.isAssociated,
      }, 'ADD', { product })
      setCartItems(item)
    }
    asyncAddToCart()
  ...
  return (
    <section aria-labelledby="related-heading" className="px-4 py-8 mt-10 border-t border-gray-200 sm:px-0">
      {Object.keys(computedItems).map((relatedItem: any, relatedItemIdx: number) => (
                    ...
                    <div className="relative w-full overflow-hidden rounded-lg h-72">
                      <Link href={`/${product.slug}`} passHref>
                        <div className="image-container">
                          <Image style={css} height={20} width={20} src={generateUri(product.image, 'h=500&fm=webp') || IMG_PLACEHOLDER}
                            alt={product.name} className="object-cover object-center w-full h-full image" />
                        </div>
                      </Link>
                      ...
                      <div className="absolute inset-x-0 top-0 flex items-end justify-end p-4 overflow-hidden rounded-lg h-72">
                        ...
                        <p className="relative text-lg font-semibold text-white">{product.price.formatted.withTax}</p>
                      </div>
                    </div>
                    <div className="mt-6">
                      ...
                      {isEngravingAvailable && (
                        <>
                          ...
                          <Engraving show={isEngravingOpen} submitForm={() => addToCart(product)} onClose={() => showEngravingModal(false)} />
                        </>
                      )}
                    </div>
                  </div>
