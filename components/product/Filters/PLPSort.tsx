  ...
  return (
    <>
      ...
      <div className="flex flex-wrap w-full">
        {!!sortList?.length ? sortList.map((option: any) => {
          option.value = option.value.replace(':', '')
          option.trimmedValue = option.value?.length > MAX_CHARS ? option.value.substring(0, MAX_CHARS) + '...' : option.value
          if (currentOption?.key === option.key) {
            return (
              <div key={option.value} className="flex items-center justify-center text-xs lg:text-sm md:text-sm mt-[1px] ml-[1px] outline outline-1 hover:z-10 hover:outline-gray-900 text-gray-900 capitalize text-center px-2 py-4 h-full w-full flex-[1_0_calc(50%-1px)] truncate outline-gray-900 z-10 cursor-default" title={option.value}>
                {option.trimmedValue}
              </div>
            )
          }
          return (
            <Link
              key={option.value}
              href={{ pathname: router.pathname, query: { ...router.query, sortBy: option.key, }, }}
              passHref
              onClick={() => {
