...
const OutOfStockFilter: React.FC<OutOfStockFilterProps> = ({
  ...
  return (
    <>
      <div className="flex flex-col py-0 pr-1 text-xs font-normal text-black font-14 whitespace-nowrap dark:text-white"> Include out of stock products </div>
      <Switch checked={!excludeOOSProduct} onChange={onEnableOutOfStockItems} className={`${excludeOOSProduct ? 'bg-gray-300' : 'bg-green'} relative mr-3 inline-flex h-[18px] w-[35px] shrink-0 border-black cursor-pointer rounded-full border transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2  focus-visible:ring-white focus-visible:ring-opacity-75`} >
