...
const Prices: FC<PricesProps> = ({ className = "w-full", price, listPrice, contentClass = "py-1 px-2 md:py-1.5 md:px-2.5 text-sm font-medium", featureToggle, }) => {
    ...
    <div className={`${className}`}>
      {featureToggle?.features?.enableMembership && (
        <div className="font-semibold font-14 text-black bg-orange-200 pt-1 pl-1 mb-2">
          ...
          <span className="pl-1 font-light text-right text-gray-900 font-10">{isIncludeVAT ? translate('label.orderSummary.incVATText') : translate('label.orderSummary.excVATText')} <a href="" className="inline-block align-middle -mt-1"><InformationCircleIcon className="w-4 h-4 text-black"/></a></span>
        </div>
      )}
      {price?.raw?.withTax != 0 ? (
