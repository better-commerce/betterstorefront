...
const transferBasketModal = ({
  ...
  const handleTransfer = () => {
    ...
    console.log(user)
    handleTransferBasket(selectedUserCart?.id, selectedUser?.userId , user?.userId , user?.companyId)
  }
          ...
          <div className="flex items-center justify-center min-h-full p-4 text-center">
            ...
            >
              <Dialog.Panel className="w-full max-w-md pb-6 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl">
                <Dialog.Title
                  as="div"
                  className="flex justify-between w-full px-6 py-3 text-lg font-medium leading-6 text-gray-900/70 border-b-2 shadow xsm:text-md border-gray-50"
                >
                ...
                <div className="px-6 mt-4">
                  <label
                    ...
                    className="block mb-2 text-sm font-medium text-gray-900"
                  >
                    {translate('label.product.messageText') }{translate('common.label.optional')}
                  </label>
                  <textarea
                    id="message"
                    rows={3}
