...
const NavigationItem: FC<NavigationItemProps> = ({ menuItem }) => {
  ...
  const renderMegaMenu = (menu: NavItemType) => {
    ...
    return (
      <li
        className={`menu-item flex-shrink-0 menu-megamenu menu-megamenu--large mt-0`}
      >
        {renderMainItem(menu)}
  ...
  const renderDropdownMenu = (menuDropdown: NavItemType) => {
    ...
    return (
      <Popover
        as="li"
        className="relative mt-0 menu-item menu-dropdown"
  ...
  switch (menuItem.type) {
    ...
    default:
      return (
        <li className="flex-shrink-0 mt-0 menu-item">{renderMainItem(menuItem)}</li>
      );
  }
};
