  ...
  return (
    <div className="px-4 py-0 mx-auto bg-white sm:py-0 sm:px-0 lg:px-0 sm:mt-2 md:w-4/5">
      {data?.productReviews?.length && (
        <>
          <div className="flex gap-8">
            <h3 className="relative pr-0 text-6xl font-bold text-black -top-1">
              {data?.ratingAverage}{' '}
              <span className="absolute bottom-0 text-xl font-semibold text-gray-400 -right-4">
                /5
              </span>
            </h3>
            <h2 className="mb-1 text-2xl font-semibold text-black">
              {translate('label.product.ratingsAndReviewsText')}{' '}
            </h2>
      ...
      {data?.productReviews?.map((review: any, reviewIdx: number) => (
        <div
          className="flex flex-col py-6 border-t border-gray-200"
        ...
        >
          ...
          <div className="flex-1">
            <h4 className="mt-1 font-semibold text-black text-md sm:mt-2">
              {review.title}
            </h4>
            <p className="mt-1 text-sm font-normal text-gray-700 sm:mt-2">
              {review.comment}
            </p>
            <h4 className="mt-1 text-xs font-medium text-gray-400 sm:mt-2">
              {review.nickName} | {review?.postedOn}
            </h4>
          </div>
        </div>
      ))}
