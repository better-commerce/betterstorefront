...
const getRecentlyViewedApiMiddleware = async (req: any, res: any) => {
  try {
    let response: any
    ...
    if (
      ...
      response = await commerce.getAllRecentlyViewedProducts({
        query: req.body,
        cookies: req.cookies,
      })
    }
...
export default apiRouteGuard(getRecentlyViewedApiMiddleware)
