  ...
  return (
    <section className="relative pb-10 text-gray-900 sm:text-sm">
      <div className="w-full px-0 mx-auto sm:container sm:px-0 lg:px-0">
        <div className="px-2 py-4 mb-4 border-b mob-header md:hidden full-m-header mx-5">
          <h3 className="max-w-4xl mt-2 mx-auto text-xl font-semibold text-black flex gap-1 mx-5">
            ...
            <Link className="mr-2 mx-2 leading-none" href="/my-account">My Saved Address</Link>
          </h3>
        </div>
                    ...
                    {item.text == 'My Saved Address' ? (
                      <>
                        <div
                          key={`my-acc-${idx}`}
                          // href="#"
                          className={`ring-white relative ring-opacity-60 border-b border-slate-300 sm:border-0 cursor-pointer ring-offset-2 ring-offset-blue-400 focus:outline-none focus:ring-2  w-full text-14  leading-5 text-left pl-2 ${item.text == "My Saved Address" ? "bg-white border-r-0  sm:border-b-0 sm:border-r-2 sm:border-black font-bold opacity-full" : "text-primary font-medium opacity-60"}`}
                        >
                          <span className="pr-2 leading-none align-middle acc-mob-icon-i sm:absolute top-2/4 -translate-y-2/4" >
                          ...
                          <Link
                            ...
                            <span className={`hidden sm:inline-block text-black dark:text-black ${item.text == 'My Saved Address' && 'font-display'}`}>{item.text}</span>
                      ...
                      <>
                        <Link
                          ...
                          <span className="inline-block sm:hidden text-black capitalize dark:text-black">
                            {item.mtext}
                          </span>
