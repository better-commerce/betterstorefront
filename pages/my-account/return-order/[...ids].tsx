...
import { NEXT_RETURN_REASON, EmptyGuid, NEXT_GET_ORDER_DETAILS, NEXT_RETURN_ORDER_LINE, Messages, } from '@components/utils/constants'
import { sanitizeBase64 } from '@framework/utils/app-util';
import { recordGA4Event } from '@components/services/analytics/ga4';
import Spinner from '@components/ui/Spinner';

export default function ReturnOrder({ orderId = EmptyGuid, itemId = EmptyGuid, deviceInfo, }: any) {
    ...
    const returnOrderLine = async (data: any) => {
        const { data: orderReturnData }: any = await axios.post(NEXT_RETURN_ORDER_LINE, data);
            ...
            selectedImages?.forEach((blob: any) => {
                promises.push(
                    new Promise((resolve, _) => {
                        ...
                        reader.onloadend = () => resolve({ name: blob?.name, data: sanitizeBase64(reader?.result as string) });
            ...
            if (orderDetails?.order?.items?.length) {
                ...
                setItemData(
                    orderDetails?.order?.items?.length > 0
                        ? orderDetails?.order?.items?.find((x: any) => matchStrings(x?.productId, itemId, true))
                        : []);
            }
            //debugger;
                                                ...
                                                <li className="px-0 pb-2 my-4">
                                                            ...
                                                            <div>
                                                                <div className="flex flex-col justify-between font-medium text-gray-900">
                                                                    ...
                                                                    <h3 className="pr-6 mt-2 font-normal text-12 text-primary">
                                                                        <Link href={`/${itemData?.slug}`} passHref className='font-semibold text-md dark:text-gray-900'>
                                                                            {itemData?.name}
                                                                        </Link>
                                                                    </h3>
                                                                    <p className="mt-2 text-sm font-medium">{itemData?.price?.formatted?.withTax}</p>
                                                                    <div className="flex mt-3">
                                                                        <div className="w-24">
                                                                            <label className="font-medium dark:text-gray-900">Size: {itemData?.size}</label>
                                                                        </div>
                                                                        <div className="w-full">
                                                                            <label className="font-medium dark:text-gray-900">Qty: {itemData?.qty}</label>
                                                                        </div>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                    ...
                                                    <div className="flex items-center justify-between pb-2 border-gray-300 border-dashed border-y">
                                                        <div className="flex items-end flex-1 px-3 py-2 pl-0 mt-1">
                                                            <label className="text-base font-bold text-primary">Select Quantity</label>
                                                        </div>
                                                        <div className="flex items-end px-3 py-2 pl-0 mt-1 ml-2">
                                                            <div className="flex items-end flex-1 px-3 py-2 mt-1 ml-2 text-sm border border-gray-200">
                                                                <label className='text-xs text-primary'>Qty: </label>
                                                                <select className="w-full px-1 text-xs bg-white sm:w-22 text-primary" value={value} onChange={handleChange}>
                                                        ...
                                                        {value > 0 ? (
                                                            <button type="button"
                                                                onClick={() => {
                                                                    onReturnReason();
                                                                    hideReturnReasons();
                                                                }} className='block w-full px-12 py-3 font-semibold text-center text-white bg-black border hover:bg-gray-800 text-14 link-btn'>Proceed to Return</button>
                                                        ) : (
                                                            <button type="button"
                                                                className='block w-full px-12 py-3 font-semibold text-center text-white bg-black border hover:bg-gray-800 text-14 link-btn'>Proceed to Return</button>
                                                        )}
