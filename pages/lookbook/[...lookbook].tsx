...
import { NEXT_BULK_ADD_TO_CART, NEXT_GET_SINGLE_LOOKBOOK } from '@components/utils/constants'
...
function LookbookDetailPage({ data, slug, deviceInfo, }: any) {
  ...
  useAnalytics(PageViewed, { eventType: PageViewed, pageCategory: 'Lookbook', omniImg: data.mainImage, })
  useEffect(() => { if (slug) loadProducts() }, [])
  ...
  const handleBulk = async () => {
    ...
    const newCart = await axios.post(NEXT_BULK_ADD_TO_CART, { basketId, products: computedProducts })
    if (newCart?.data) {
    ...
    <div className="flex flex-col items-center w-full px-4 py-0 pb-24 mx-auto text-left bg-white sm:px-0 lg:px-0">
      <div className="w-full overflow-hidden bg-gray-200 rounded-sm">
        <div className="image-container lookbook-image">
          <Image style={css} width={1000} height={400} src={generateUri(data.mainImage, 'h=700&fm=webp') || IMG_PLACEHOLDER}
            alt={data.name} className="object-cover object-center w-full h-screen min-h-screen image" />
          <div className="lookbook-data">
            <h1 className="py-1 text-4xl font-semibold tracking-tight text-white">{data.name}</h1>
            <h2 className="pt-4 font-normal tracking-tight text-white text-md sm:w-1/3">{data.description}</h2>
            <button onClick={handleBulk} className="px-10 py-3 mt-5 text-lg font-semibold text-white uppercase bg-black hover:bg-gray-900">
              {SHOP_THE_LOOK}
            </button>
          </div>
        </div>
...
export async function getStaticProps({ params, locale, locales, preview }: GetStaticPropsContext) {
  ...
  return { props: { data: response, slug: slug[0], globalSnippets: infra?.snippets ?? [], snippets: response?.snippets }, revalidate: 200, }
}
